...
async function updateEmployeeTotals(employeeId: string) {
  ...
  const { error: employeeError } = await supabase
    .from("employees")
    .update({
      sick_leave_total: totalLeaves,
      sick_leave_used: usedLeaves,
      sick_leave_balance: balanceLeaves,
    })
    .eq("id", employeeId);
  ...
  try {
    ...
    if (newPeriodBalance < 0) {
      return NextResponse.json(
        {
          error:
            `Period ${periodYear} has insufficient Sick Leave balance.`,
        },
        { status: 400 }
      );
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: `Sick Leave period ${periodYear} deleted successfully.`,
      employee_totals: employeeTotals,
    });
  } catch (error) {
    return NextResponse.json(
