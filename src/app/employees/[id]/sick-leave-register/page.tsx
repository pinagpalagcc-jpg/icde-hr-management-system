...
export default function AnnualLeaveRegisterPage({
    ...
    try {
      ...
      const [employeeResponse, registerResponse] =
        await Promise.all([
          ...
          fetch(
            `/api/sick-leave-transactions?employee_id=${encodeURIComponent(
              id
            )}`,
            {
              cache: "no-store",
      ...
      if (!registerResponse.ok) {
        throw new Error(
          registerData?.error ||
            "Unable to load Sick Leave Register."
        );
      }
  ...
  async function deletePeriod(periodYear: number) {
    const confirmed = window.confirm(
      `Delete Sick Leave period ${periodYear}? This will delete all transactions for this period.`
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || "Unable to delete Sick Leave period."
        );
      }
      ...
      setSuccessMessage(
        `Sick Leave period ${periodYear} deleted successfully.`
      );
    } catch (error) {
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Unable to delete Sick Leave period."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function saveTransaction() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error ||
            "Unable to save Sick Leave transaction."
        );
      }
      ...
      setSuccessMessage(
        isAdd
          ? `Sick Leave period ${year} added successfully.`
          : `${days} Sick Leave day(s) encashed from period ${year}.`
      );
              ...
              <tbody>
                {periods.length === 0 ? (
                  <tr>
                    ...
                    >
                      No Sick Leave periods have been added.
                    </td>
                  </tr>
                ) : (
                  periods.map((period) => (
