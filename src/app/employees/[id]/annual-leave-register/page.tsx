...
export default function AnnualLeaveRegisterPage({
  ...
  async function deletePeriod(periodYear: number) {
    const confirmed = window.confirm(
      `Delete Annual Leave period ${periodYear}? This will delete all transactions for this period.`
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || "Unable to delete Annual Leave period."
        );
      }
      ...
      setSuccessMessage(
        `Annual Leave period ${periodYear} deleted successfully.`
      );
    } catch (error) {
      setErrorMessage(
        error instanceof Error
