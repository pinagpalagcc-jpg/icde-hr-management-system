...
export default function EmployeeProfilePage({
  ...
  async function deleteDocument(documentId: string) {
    ...
    if (password !== ADMIN_DELETE_PASSWORD) return alert("Wrong password. Document was not deleted.");
    if (!confirm("Delete this document permanently?")) return;

    const res = await fetch(`/api/employee-documents/${documentId}`, { method: "DELETE" });
    if (!res.ok) {
      const result = await res.json();
      ...
      <main className="flex-1 p-8 overflow-x-hidden">
            ...
            <div className="flex items-center gap-6">
              <div>
                <div className="w-32 h-32 rounded-2xl bg-[#3f4447] text-white flex items-center justify-center text-4xl font-bold overflow-hidden">
                  {employee.profile_photo ? <img src={employee.profile_photo} alt="Employee Photo" className="w-full h-full object-cover" /> : initials}
                </div>
                ...
                {employee.profile_photo && <button onClick={removePhoto} className="mt-2 w-full text-sm bg-red-100 text-red-700 px-3 py-2 rounded-xl font-semibold">Remove Photo</button>}
              </div>
              ...
              <div>
                ...
                <span className={`inline-block mt-3 px-4 py-2 rounded-full font-semibold ${(employee.status || "Available") === "On Leave" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
            ...
            <div className="flex flex-wrap gap-3">
              {editMode ? (
                <>
                  ...
                  <button onClick={saveChanges} disabled={saving} className="bg-[#d2b241] text-white px-5 py-3 rounded-xl font-semibold">{saving ? "Saving..." : "Save Changes"}</button>
                </>
              ) : (
                <button onClick={() => setEditMode(true)} className="bg-[#d2b241] text-white px-5 py-3 rounded-xl font-semibold">Edit Profile</button>
              )}

              <button onClick={toggleStatus} className={(employee.status || "Available") === "On Leave" ? "bg-green-100 text-green-700 px-5 py-3 rounded-xl font-semibold" : "bg-orange-100 text-orange-700 px-5 py-3 rounded-xl font-semibold"}>
        ...
        {activeTab === "Personal Information" && (
          editMode ? (
            ...
            <InfoWide title="Personal Information" rows={[
        ...
        {activeTab === "Employment Detail" && (
          editMode ? (
            <EditSection title="Employment Detail">
              ...
              <button type="button" onClick={() => update("must_change_password", "true")} className="bg-orange-100 text-orange-700 px-5 py-3 rounded-xl font-semibold mt-7">Force Password Change</button>
            </EditSection>
          ) : (
            <InfoWide title="Employment Detail" rows={[
        ...
        {activeTab === "Salary and Benefits" && (
          editMode ? (
            ...
            <InfoWide title="Salary and Benefits" rows={[
        ...
        {activeTab === "Office Documents" && <DocumentCenter category="Office Documents" docForm={docForm} setDocForm={setDocForm} handleFile={handleFile} uploadDocument={uploadDocument} documents={documents} deleteDocument={deleteDocument} />}
        {activeTab === "Immigration Documents" && <DocumentCenter category="Immigration Documents" docForm={docForm} setDocForm={setDocForm} handleFile={handleFile} uploadDocument={uploadDocument} documents={documents} deleteDocument={deleteDocument} />}
        {activeTab === "Personal Documents" && <DocumentCenter category="Personal Documents" docForm={docForm} setDocForm={setDocForm} handleFile={handleFile} uploadDocument={uploadDocument} documents={documents} deleteDocument={deleteDocument} />}
      ...
      <div className="bg-[#f7f4ec] rounded-2xl p-5 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <label className="flex items-center gap-3 bg-white rounded-xl border px-4 py-3 mt-7">
            <input type="checkbox" checked={docForm.not_applicable} onChange={(e) => setDocForm({ ...docForm, not_applicable: e.target.checked, issue_date: e.target.checked ? "" : docForm.issue_date, expiry_date: e.target.checked ? "" : docForm.expiry_date })} />
            ...
            {rows.length ? rows.map((doc: any) => {
              ...
              return (
                <tr key={doc.id} className="border-b hover:bg-[#f7f4ec]">
                  ...
                  <td className="p-3">{doc.file_data ? <button type="button" onClick={() => { const w = window.open("", "_blank"); if (w) { w.document.write(`<html><head><title>${doc.document_name}</title></head><body style="margin:0"><iframe src="${doc.file_data}" style="width:100%;height:100vh;border:0"></iframe></body></html>`); w.document.close(); } }} className="text-[#d2b241] font-bold">Preview</button> : "-"}</td>
              ...
              <tr><td colSpan={8} className="p-5 text-center text-gray-500">No {category.toLowerCase()} uploaded yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function InfoWide({ title, rows }: { title: string; rows: string[][] }) {
  return <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6"><h2 className="text-xl font-bold text-[#3f4447] mb-5">{title}</h2>{rows.map(([l, v]) => <div key={l} className="flex justify-between border-b py-2 gap-4"><span className="text-gray-500">{l}</span><b className="text-right">{v}</b></div>)}</section>;
}
...
function EditSelect({ label, field, value, options, update }: any) {
  return <div><label className="text-sm font-semibold text-gray-600">{label}</label><select value={value || ""} onChange={(e) => update(field, e.target.value)} className="mt-2 w-full border rounded-xl px-4 py-3 outline-none bg-white"><option value="">Select</option>{options.map((o: string) => <option key={o}>{o}</option>)}</select></div>;
}
